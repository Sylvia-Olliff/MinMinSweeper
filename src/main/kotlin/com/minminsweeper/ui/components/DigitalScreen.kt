...
internal fun DigitalScreen(
    ...
    Row(
        modifier = Modifier
            ...
            .background(MinMinSweeperColors.onPrimary),
...
private fun SegmentDigit(
    ...
    Canvas(
        Modifier
            ...
            .padding(tinyPad / 2)
            .requiredSize(windowPad, headerHeight)
    ) {
        ...
        for ((i, path) in segments.withIndex()) {
            drawPath(path = path, color = MinMinSweeperColors.onError, alpha = if (segment.isActive(i)) 1f else 0.15f)
        }
    }
}
